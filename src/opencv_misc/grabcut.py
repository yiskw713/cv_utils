...
class GrabCut(object):
    ...
    def demo(self, img_path:str, foreground_rect:Tuple[int, int,int, int]) -> None:
        ...
        cv2.grabCut(img, self.mask, foreground_rect, self.bg_model, self.fg_model, self.n_iteration, self.mode)

        # remove background
        result_mask = np.where((self.mask == 2) | (self.mask == 0), 0, 1).astype(np.uint8)
